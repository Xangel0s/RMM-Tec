<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('roles', function (Blueprint $table) {
            $table->id();
            $table->string('name'); // admin, technician, viewer
            $table->json('permissions'); // ["view:endpoints", "execute:commands", "install:software"]
            $table->timestamps();
        });

        Schema::create('endpoint_groups', function (Blueprint $table) {
            $table->id();
            $table->string('name'); // "Departamento IT"
            $table->text('description')->nullable();
            $table->json('endpoints')->nullable(); // [uuid1, uuid2]
            $table->timestamps();
        });

        Schema::create('user_group_permissions', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user_id')->constrained('users')->cascadeOnDelete();
            $table->foreignId('group_id')->constrained('endpoint_groups')->cascadeOnDelete();
            $table->foreignId('role_id')->constrained('roles')->cascadeOnDelete();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('user_group_permissions');
        Schema::dropIfExists('endpoint_groups');
        Schema::dropIfExists('roles');
    }
};
